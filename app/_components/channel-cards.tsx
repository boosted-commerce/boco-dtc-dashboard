  ...
  return (
    <div className="mb-6">
      ...
      <section className="mb-3 grid grid-cols-1 gap-3 md:grid-cols-2">
        <Card title={`Sessions · ${period}d${suffix}`} bucket={sessions} kind="count" color="text-sky-600 dark:text-sky-400" bands={bands} channelMode={channelMode} />
        <Card title={`Conversion · ${period}d${suffix}`} bucket={convRate} kind="percent" color="text-indigo-600 dark:text-indigo-400" bands={bands} channelMode={channelMode} />
      </section>
      <section className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <Card title={`Orders · ${period}d${suffix}`} bucket={orders} kind="count" color="text-emerald-600 dark:text-emerald-400" bands={bands} channelMode={channelMode} />
        <Card title={`Revenue${allocated} · ${period}d${suffix}`} bucket={revenue} kind="currency" color="text-emerald-600 dark:text-emerald-400" bands={bands} channelMode={channelMode} />
        <Card title={`AOV${allocated} · ${period}d${suffix}`} bucket={aov} kind="aov" color="text-blue-600 dark:text-blue-400" bands={bands} channelMode={channelMode} />
        {!channelMode && (
          <Card title={`Subscription rev (landed here) · ${period}d`} bucket={all.subRevenue} kind="currency" color="text-purple-600 dark:text-purple-400" bands={bands} channelMode={false} />
        )}
      </section>
      {channelMode && (
