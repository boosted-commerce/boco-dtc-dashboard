...
// Add a page to the manually-curated Landing Pages list by URL/path.
// Mirrors AddPinnedInput but targets /api/lp/toggle.
export function AddLPInput({ brand }: { brand: string }) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        const res = await fetch('/api/lp/toggle', {
          method: 'POST',
...
// Per-row remove control shown on Landing Pages rows.
export function RemoveLPButton({ brand, path }: { brand: string; path: string }) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  ...
  return (
    <button
      ...
      onClick={remove}
      disabled={pending}
      title="Remove from Landing Pages list"
      aria-label={`Remove ${path} from Landing Pages`}
